...
func step(name string) {
	...
	fmt.Printf("%-46s live=%7.1f MiB  delta=%+8.1f  RSS=%7.1f  peak=%7.1f  took=%8s\n",
		name, live, live-lastLive, meminfo.RSS(), meminfo.PeakRSS(), work.Round(time.Millisecond))
	lastLive = live
	// STOP_AFTER_STEP=<n> stops the program right after the step whose label
	// starts with "<n>.", so the process counters can be read with only that
	// much of the startup path executed.
	if n := os.Getenv("STOP_AFTER_STEP"); n != "" && strings.HasPrefix(name, n+".") {
		reportProcess("stopped after step " + n)
		os.Exit(0)
	}
	begin()
}
...
func main() {
	...
	if os.Getenv("PHASES_ONLY") != "" {
		// Run nothing but the scope-independent parse phases, in an otherwise
		// untouched process, so the arena they alone force can be read.
		begin()
		step("0. process start (binary linked, inits run)")
		measureScopeIndependentPhases()
		reportProcess("after P1-P3 only")
		return
	}
	...
	fmt.Printf("   family filter (UPJET_FAMILY_FILTER)  : %q\n", os.Getenv("UPJET_FAMILY_FILTER"))
	fmt.Printf("   GOGC=%q GOMEMLIMIT=%q\n", os.Getenv("GOGC"), os.Getenv("GOMEMLIMIT"))
	...
	if os.Getenv("STOP_AFTER_STARTUP") != "" {
		return
	}
...
// reportProcess prints the whole-process memory counters: the Go MemStats
// arena figures and the /proc smaps_rollup breakdown, which are different
// things - HeapAlloc is live data, HeapSys is the arena the runtime has
// reserved for the heap, and smaps Anonymous is what the kernel has actually
// backed with pages (heap plus stacks), which is the part a pod's
// container_memory_working_set_bytes counts in full.
func reportProcess(label string) {
	var ms runtime.MemStats
	runtime.ReadMemStats(&ms)
	mib := func(v uint64) float64 { return float64(v) / (1 << 20) }
	fmt.Printf("\n%s\n", label)
...
// dumpNames writes "<terraform name>\t<short group>" for every configured
// resource, sorted, so that a filtered run's resource set can be diffed
// against the family subset of an unfiltered run's.
func dumpNames(p *config.Provider, path string) {
	lines := make([]string, 0, len(p.Resources))
	for n, r := range p.Resources {
		lines = append(lines, fmt.Sprintf("%s\t%s", n, r.ShortGroup))
	}
	sort.Strings(lines)
