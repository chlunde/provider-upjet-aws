...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_applicationinsights(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_applicationinsights(mgr), "Cannot setup namespaced webhooks")
	}
