...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_cur(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_cur(mgr), "Cannot setup namespaced webhooks")
	}
