...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-aws-networkmonitor",
		Cache: cache.Options{
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_networkmonitor(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_networkmonitor(mgr), "Cannot setup namespaced webhooks")
	}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_networkmonitor(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_networkmonitor(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_networkmonitor(mgr, clusterOptions), "Cannot setup cluster-scoped AWS controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_networkmonitor(mgr, namespacedOptions), "Cannot setup namespaced AWS controllers")
	}
