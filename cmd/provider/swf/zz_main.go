...
func main() { //nolint:gocyclo // easier to follow as a unit
	...
	if startWebhooks {
		kingpin.FatalIfError(clustercontroller.SetupWebhookWithManager_swf(mgr), "Cannot setup cluster-scoped webhooks")
		kingpin.FatalIfError(namespacedcontroller.SetupWebhookWithManager_swf(mgr), "Cannot setup namespaced webhooks")
	}
